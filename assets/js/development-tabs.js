/**
 * wpseed Development Tabs JavaScript
 *
...
(function($) {
    ...
    function initdevelopment() {
        console.log('wpseed Development Tabs JS loaded');
    ...
    function initUILibrary() {
        ...
        if (typeof wpseedUILibrary !== 'undefined') {
            // UI Library JavaScript is loaded
            console.log('wpseedUILibrary object detected');
        }
    }
...
jQuery(document).ready(function($) {
    ...
    $('.refresh-github-data').on('click', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wpseed_refresh_github_data',
                nonce: wpseed_dev_tabs.nonce
            },
    ...
    $('.section-toggle-checkbox').on('change', function() {
        ...
        localStorage.setItem('wpseed_ui_library_sections', JSON.stringify(sectionStates));
    });
    ...
    var savedStates = localStorage.getItem('wpseed_ui_library_sections');
    if (savedStates) {
        try {
    ...
    $('.move-to-assets').on('click', function() {
        ...
        if (confirm(wpseedData.confirmMoveFile || 'Are you sure you want to move this file to the assets folder?')) {
            $btn.prop('disabled', true).text(wpseedData.movingText || 'Moving...');
            
            setTimeout(function() {
                alert(wpseedData.moveFileMessage || 'File move functionality would be implemented here');
                $btn.prop('disabled', false).text(wpseedData.moveToAssetsText || 'Move to Assets');
            }, 1000);
        }
    });
    ...
    $('.delete-file').on('click', function() {
        ...
        if (confirm(wpseedData.confirmDeleteFile || 'Are you sure you want to delete this file? This action cannot be undone.')) {
            $btn.prop('disabled', true).text(wpseedData.deletingText || 'Deleting...');
            
            setTimeout(function() {
                alert(wpseedData.deleteFileMessage || 'File delete functionality would be implemented here');
                $btn.prop('disabled', false).text(wpseedData.deleteText || 'Delete');
            }, 1000);
        }
    });
    ...
    $('#load-log-file').on('click', function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wpseed_load_log_file',
                log_file: logFile,
                security: wpseed_dev_tabs.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Tasks functionality
    if (typeof wpseedTasksData !== 'undefined') {
        $('.start-task').on('click', function(e) {
            e.preventDefault();
            var taskId = $(this).data('task-id');
            var taskSource = $(this).data('task-source');
            
            localStorage.setItem('wpseed_current_task_id', taskId);
            localStorage.setItem('wpseed_current_task_source', taskSource);
            
            $(this).closest('tr').attr('data-status', 'in-progress');
            
            if (taskSource !== 'github') {
                var taskData = null;
                for (var i = 0; i < wpseedTasksData.tasks.length; i++) {
                    if (wpseedTasksData.tasks[i].id === taskId) {
                        taskData = wpseedTasksData.tasks[i];
                        break;
                    }
                }
                ...
                if (taskData) {
                    ...
                    $.ajax({
                        url: wpseed_dev_tabs.ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'wpseed_create_github_issue',
                            nonce: wpseed_dev_tabs.nonce,
                        ...
                        success: function(response) {
                            if (response.success) {
                                localStorage.setItem('wpseed_GITHUB_issue_number', response.data.number);
                                localStorage.setItem('wpseed_GITHUB_issue_url', response.data.html_url);
                                window.location.href = 'admin.php?page=wpseed_development&tab=current_task';
                            } else {
