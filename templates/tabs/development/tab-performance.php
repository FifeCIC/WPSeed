...
class WPSeed_Admin_Development_Performance {
    ...
    public static function output() {
        ...
        if (isset($_POST['clear_logs']) && isset($_POST['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce'])), 'wpseed_clear_logs')) {
            WPSeed\Core\Enhanced_Logger::clear_old_logs(0);
            echo '<div class="notice notice-success"><p>' . esc_html__('Logs cleared successfully.', 'wpseed') . '</p></div>';
        }
        
        $logs = WPSeed\Core\Enhanced_Logger::get_recent_logs(50);
        $current_metrics = WPSeed\Core\Enhanced_Logger::instance()->get_performance_metrics();
        ...
        <div class="wpseed-performance-monitor">
            ...
            <!-- Query Statistics -->
            <?php 
            $query_stats = WPSeed\Core\Enhanced_Logger::instance()->get_query_stats();
            ...
            <!-- Hook Statistics -->
            <?php $hook_stats = WPSeed\Core\Enhanced_Logger::instance()->get_hook_stats(); ?>
