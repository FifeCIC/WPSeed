...
class WPSeed_Admin_Setup_Wizard {
    ...
    public function setup_wizard_header() {        
        ...
        <head>
            ...
            <title><?php esc_htmlesc_html_e( 'WordPress Seed &rsaquo; Setup Wizard', 'wpseed' ); ?></title>
    ...
    public function setup_wizard_footer() {
        ?>
            <?php if ( 'next_steps' === $this->step ) : ?>
                <a class="wpseed-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_htmlesc_html_e( 'Return to the WordPress Dashboard', 'wpseed' ); ?></a>
            <?php endif; ?>
            </body>
        </html>
        <?php
    ...
    public function setup_wizard_content() {           
        ...
        if( !isset( $this->steps[ $this->step ]['view'] ) ) {
            ?><h1><?php esc_htmlesc_html_e( 'Invalid Step!', 'wpseed' ); ?></h1><p><?php esc_htmlesc_html_e( 'You have attempted to visit a setup step that does not exist. I would like to know how this happened so that I can improve the plugin. Please tell me what you did before this message appeared. If you were just messing around, then stop it you naughty hacker!', 'wpseed' ); ?></p><?php 
        } elseif( !method_exists( $this, $this->steps[ $this->step ]['view'][1] ) ) {
            ?><h1><?php esc_htmlesc_html_e( 'Something Has Gone Very Wrong!', 'wpseed' ); ?></h1><p><?php esc_htmlesc_html_e( 'You have attempted to visit a step in the setup process that may not be ready yet! This should not have happened. Please report it to me.', 'wpseed' ); ?></p><?php             
        } else {
    ...
    public function wpseed_setup_introduction() { ?>
        <h1><?php esc_htmlesc_html_e( 'Setup WordPress Seed', 'wpseed' ); ?></h1>
    ...
    public function wpseed_setup_administrators() { 
        ...
        <form method="post">
            ...
            <p class="wpseed-setup-actions step">
                <input type="submit" class="button-primary button button-large button-next" value="<?php esc_attresc_html_e( 'Continue', 'wpseed' ); ?>" name="save_step" />
